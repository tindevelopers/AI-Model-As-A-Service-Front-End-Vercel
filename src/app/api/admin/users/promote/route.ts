...
// POST: Promote user to admin (superadmin only)
export async function POST(request: NextRequest) {
  try {
    ...
    // Call the promote_user_to_admin function (superadmin only)
    const { data, error } = await supabase.rpc('promote_user_to_admin', {
      target_user_id: targetUserId
    })

    if (error) {
      errorLogger.logError('Failed to promote user to admin', {
        component: 'admin-users-promote-route',
        action: 'POST',
        userId,
        additionalData: {
          error: error.message,
