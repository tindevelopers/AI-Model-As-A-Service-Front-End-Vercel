  ...
  constructor(config: RateLimitConfig) {
    super({
      ...
      keyGenerator: (request: NextRequest) => {
        ...
        const ip = this.getClientIP(request)
        return `user_rate_limit:${userId || ip}`
      }
    })
  }

  private getUserIdFromRequest(request: NextRequest): string | null {
    // Try to extract user ID from headers or other sources
...
export const userRateLimiters = {
  blogGeneration: new UserRateLimiter({
    windowMs: 24 * 60 * 60 * 1000, // 24 hours
    maxRequests: 50 // 50 blog generations per day per user
  }),

  apiUsage: new UserRateLimiter({
    windowMs: 60 * 60 * 1000, // 1 hour
    maxRequests: 1000 // 1000 API calls per hour per user
  })
}
